<template>
  <div class="max-w-md mx-auto p-6 relative mb-10">
    ...
    <!-- Star badge -->
    <div class="flex justify-end mb-4 absolute top-[72px] right-6">
      <button
        type="button"
    ...
    <div class="flex justify-end mb-2 absolute top-[132px] right-6">
    ...
    <div class="flex items-center justify-between space-x-6 pr-4 w-full">
      ...
      <div class="grid grid-cols-[auto_1fr] gap-x-3 gap-y-2 text-[12px] text-gray-900">
        <template v-for="it in assetPrefs" :key="it.label">
          ...
          <div class="text-right">{{ it.value.toFixed(2) }}%</div>
        </template>
      </div>
    </div>
...
const assetPrefs = [
  { label: 'BTC', value: 52.1, color: '#2563EB' },
